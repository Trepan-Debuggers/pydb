% $Id: libpydb.tex,v 1.78 2006/09/17 09:42:48 rockyb Exp $
...
switched to safely (and for all practical purposes this means they've
been blocked by the debugger.

One will get the most information using Python 2.5 or having
threadframe installed. However since the debugger keeps its own
information about threads, one can still get some information when
...
\subsubsection{Setting the frame to a non-current thread ({\tt frame})}

\subsubsection{Setting a breakpoint at a specific thread ({\tt break})\label{command:break-thread}}
