...
interface \ulink{\tt ddd}{http://www.ddd.org}, basically all I had do
do was tell {\tt ddd} that handling this construct (e.g. stepping) is like
gdb. There were a few places where I told {\tt ddd} not to follow {\tt gdb} but
Perl instead because the paradigm had to be more like a scripting
language than a compiled language. But in the end, adding support for
the bash debugger inside ddd was much more straightforward and
required much less thought than if I had invented my own debugger
command set.

After this was done and I fire up {\tt ddd}, I noticed that when my
...
or remember modifying any code for using tool tips. How did {\tt ddd} do
this? I was very curious and had to find out.

Because I even copied the output format of gdb's ``info'', ``set'' and
``show'' commands, {\tt ddd} ran these commands on its own and parsed
the output; it then used that output to form tool tips and create a
customization boxes!

In a preliminary posting to {\tt comp.lang.python} asking why the
...
There are two extension we've provided in running the debugger we have
provided. First this debugger should installlitself somewhere in you
command path, usually as \samp{pydb}, so you don't have to invoke it
as an argument to the \samp{python} command. 
...
In particular \samp{pydb} has the two very common options
...
\samp{ pydb} \optional{\var{debugger-options}\ldots} \var{python-script} \optional{\var{script-options}\ldots}
...
may want to use this option. The \samp{--linetrace} option implicitly
sets this option.

\item[--command=\var{filename}\code{\Large{|}}-x \var{filename}]
...
option for this is \samp{-x}; however since we follow \samp{gdb}
conventions \samp{-x} is used as a short option for
\samp{--command}. When line tracing is turned on, each location (file
name and linenumber) is printed before the command is executed. This
option can be used in conjuntion with the \samp{--output} and
\samp{--error} options described above situations where a terminal is
