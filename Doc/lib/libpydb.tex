% $Id: libpydb.tex,v 1.31 2006/03/13 15:09:40 rockyb Exp $
...
number shown.  This command is available only if a readline module
is available and supports the history saving.
...
{\tt help}, {\tt quit}, {\tt unalias})\label{subsection-misc}}
...
If readline is available on your OS, the \code{complete} command will
print a list of command names that start with \var{command-prefix}.

In addition the command-completion key (usually the tab key) can be
used to complete command names.
...
\code{pydb} function \code{set_trace()} can be used. I've even this
...
in a file called \code{hardtodebug.py}:
...
Also, if readline support is available on your OS, \class{Cmd} will
use that, and both command history and command completion are thus
available in the debugger. The method the command that corresponds to
\code{complete} (see \ref{command:complete}, \code{do_complete}, will
also be available.
