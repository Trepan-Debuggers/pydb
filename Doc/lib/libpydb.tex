% $Id: libpydb.tex,v 1.11 2006/02/17 02:31:17 rockyb Exp $
...
\code{--linetrace} option implicitly sets this option.
...
Show the current line number in source file
...
return value. See also \ref{command:retval}.
...
using the \samp{finish} command) or stepping after a return statement.
...
\code{pydb.py} can also be invoked as
...
On systems that support it, \code{pydb} is a symbolic link to
\code{pydb.py} which has been arranged to be executable and contains
...
the statement using \samp{step} or \samp{next} (these commands are
