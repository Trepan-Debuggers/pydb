% $Id: libpydb.tex,v 1.79 2006/09/17 13:47:36 rockyb Exp $
...
\subsubsection{Intercepting Signals ({\tt handle})}
...
on, since it adds a bit of additional complexity. To turn it on, pass
the \code{--threading} option on the \code{pydb} invocation.

Tests have shown that one can easily cause a deadlock in trying to
change the behavior if one is not careful.

So we generally take a conservative approach. We will allow switching
threads only between those known to be in a state that they can be
switched to safely; for all practical purposes this means they've been
...
<_MainThread(MainThread, started)>
    <module>() called from file 'q.py' at line 52
\end{verbatim}

If \code{terse} is appended we just list the thread name and thread
...
The arrow indicates the current thread; here it is Thread-3. This
...
When thread debugging is turned on you can also specify whether or not
a breakpoint should occur at a point across all threads or only on a
specific thread.

If Python version 2.5 or greater is installed or
\ulink{\module{threadframe}}{http://www.majid.info/mylos/stories/2004/06/10/threadframe.html}
with a older vrsion of Python is installed, you can inspect and change
local variables from other thread frames.

\begin{description}
\item[frame \optional{\var{Thread-Name}} \var{frame-number}]
Move the current frame to the specified frame number. If a
\var{Thread-Name} is given, move the current frame to that.
\item[b(reak)
...
See \ref{command:break}. If a specific thread name is given then a
  breakpoint will occur only when in that thread.
