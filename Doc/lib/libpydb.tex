% $Id: libpydb.tex,v 1.83 2006/09/25 23:46:40 rockyb Exp $
...
on, since it adds a bit of additional complexity. For example, you may
need to issue an additional \code{step} command to get into your
program initially.
...
<Consumer(Thread-3, started)>
    run(self=<Consumer(Thread-3, star...) called from file 'q.py' at line 38
------------------------------
<Consumer(Thread-2, started)>
    get(self=<Queue.Queue instance at...) called from file 'Queue.py' at line 165
------------------------------
<Producer(Thread-1, started)>
    run(self=<Producer(Thread-1, star...) called from file 'q.py' at line 21
<_MainThread(MainThread, started)>
   MainThread: -1209960784
-> Thread-1: -1212503136
   Thread-2: -1220895840
   Thread-3: -1229288544
\end{verbatim}

If \code{terse} is appended, we just list the bottom portion: the thread
name and thread id. Here's an example:
...
can whether or not a breakpoint should occur at a point across all
threads or only on a specific thread. 

And iff Python version 2.5 or greater is installed or
\ulink{\module{threadframe}}{http://www.majid.info/mylos/stories/2004/06/10/threadframe.html}
