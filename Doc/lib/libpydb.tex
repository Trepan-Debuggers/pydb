% $Id: libpydb.tex,v 1.30 2006/03/05 00:52:28 rockyb Exp $
...
number shown.  This command Is available only if the readline module
available and supports the history saving.
...
Although it shouldn't come as any surprise, run issuing \code{pydb
myscript.py} you are running \code{pydb} first which then invokes your
script \code{myscript.py}. The debugger, \code{pydb}, tries hard to
make itself transparent and thus strips its own program options,
resets \code{sys.argv} and sets \code{__file__} to the values you
