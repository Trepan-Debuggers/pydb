% $Id: libpydb.tex,v 1.24 2006/02/27 10:18:45 rockyb Exp $
...
which then invokes your script. \code{pydb} tries hard to make itself
transparent and thus strips its own program options and resets
\code{sys.argv} and sets \code{__file__} to the values you would get by
calling the debugged program directly.
...
\code{pydb} function \code{set_trace()} can be used. 
...
Note that we stopped \emph{after\/} the return from
\code{test}. That's because that where is the next executable
statement. If you want to stop inside \code{test()} put a statement
after that like \code{return}. Furthermore, if the program had ended
at line 8, then \emph{no\/} stopping would have occured becuase the
end of the program is reached first.\footnote{Perhaps this is a
deficiency of the debugger. I'm not sure what the right way to address
though. One could have another routine to stop at a return which would
then skip intervening statements. Another possibility is to add a
routine to have debugger stop inside the call, e.g. \code{set_trace}
above. This is not hard to code and I've done so, but I'm not sure
that doesn't just confuse things more.}
