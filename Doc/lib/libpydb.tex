% $Id: libpydb.tex,v 1.17 2006/02/23 12:09:55 rockyb Exp $
...
A {\tt set} command modifies parts of the debugger environment.  can
...
\code{pydb} function \code{debugger()} can be used. For example:
...
before the next statement. However in contrast to \code{pydb.debugger}
quitting causes the program to be terminated.

If you don't want the program to be stopped when the debugger is
exited you can use the \code{pydb.debugger()} function instead. But
here the program will be stopped inside the \code{debugger}
function. The stack frame will show \code{pydb.debugger} as the most
recent entry and below that will be the program that it was called
from.

To continue running the program, issue a debugger \code{quit} or
\code{continue} command or give a termanal EOF.

A downside of this approache is that debugger session information
can't be saved between calls---each call is a new instance of the
debugger and when the debugger is left via \code{quit} the instance is
destroyed. (In the case of \code{pydb.set_trace()} the issue never
comes up because the program is terminated on exit.
...
And another way to invoke:

\begin{verbatim}
python -m pydb python-script
\end{verbatim}
...
\begin{funcdesc}{debugger}{}
Enter the debugger at the calling stack frame.  This is useful to
hard-code a breakpoint at a given point in a program, even if the code
is not otherwise being debugged (e.g., when an assertion fails).

The call stack will show \code{debugger} as the most recent call and
the next most-recent entry will be the program being debugged. See
also \code{set_trace}.
