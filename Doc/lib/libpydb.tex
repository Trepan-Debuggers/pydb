% $Id: libpydb.tex,v 1.36 2006/03/25 18:15:10 rockyb Exp $
...
has changed and thus not reimport it, so in such a situation one must
use \code{restart} rather than \code{run}.\footnote{It may be possible to
unimport by removing a the module from a namespace, but if there
are shared dynamically loaded objects those don't get unloaded.}
...
for \code{run}. Note that there may be situations where \code{run} doesn't
have the effect you want and you may have to use \code{restart}
instead.
